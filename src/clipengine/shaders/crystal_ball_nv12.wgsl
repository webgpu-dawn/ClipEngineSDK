// Crystal Ball NV12 Shader (Inverse stereographic projection from north pole)
// BT.709 color space conversion with fisheye/crystal ball mapping
...
// Convert screen UV to equirectangular UV using inverse stereographic projection (north pole)
fn toCrystalBall(uv: vec2f, yaw: f32, pitch: f32, zoom: f32, aspect: f32) -> vec2f {
    ...
    // Inverse stereographic projection from north pole (r = tan((PI - theta)/2))
    let r = sqrt(x * x + y * y);
    ...
    // Convert to spherical coordinates (viewing from above/north pole)
    let theta = 3.14159265 - 2.0 * atan(r); // Latitude from north pole
...
fn fs(input : VertexOutput) -> @location(0) vec4f {
    ...
    let crystalUV = toCrystalBall(input.uv, yaw, pitch, zoom, aspect);

    // Sample NV12 textures (must be before any non-uniform control flow)
    let y = textureSample(yTex, mySampler, crystalUV).r;
    let uv_chroma = textureSample(uvTex, mySampler, crystalUV).rg;
    ...
    let finalColor = select(rgb, vec3f(0.0), crystalUV.x < 0.0);
